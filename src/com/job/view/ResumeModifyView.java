...
import com.job.controller.ResumeModifyController;
...
public class ResumeModifyView extends JPanel{
	...
	public ResumeModifyView(Run win) {
		this.win = win;
		initialize();
	}
	...
	private void initialize() {
		...
		comboAcademic.setModel(new DefaultComboBoxModel(new String[] {"[\uCD5C\uC885\uD559\uB825]", "\uC911\uC878", "\uACE0\uC878", "\uC804\uBB38\uB300\uC878", "4\uB144\uC81C\uC878"}));
		comboAcademic.setToolTipText("");
		comboAcademic.setBounds(440, 180, 150, 30);
		add(comboAcademic);

		comboTypeOccup = new JComboBox();
		comboTypeOccup.setModel(new DefaultComboBoxModel(new String[] {"[\uADFC\uBB34\uD615\uD0DC]", "\uB9E4\uC7A5\uAD00\uB9AC", "\uC8FC\uBC29", "\uBBF8\uB514\uC5B4", "\uC6B4\uC804", "\uC0AC\uBB34", "IT", "\uC601\uC5C5", "\uAD50\uC721"}));
		comboTypeOccup.setBounds(440, 264, 150, 30);
		add(comboTypeOccup);

		comboPeriod = new JComboBox();
		comboPeriod.setModel(new DefaultComboBoxModel(new String[] {"[\uADFC\uBB34\uAE30\uAC04]", "\uD558\uB8E8", "\uC77C\uC8FC\uC77C", "1\uAC1C\uC6D4 \uC774\uC0C1", "3\uAC1C\uC6D4 \uC774\uC0C1", "6\uAC1C\uC6D4 \uC774\uC0C1"}));
		comboPeriod.setBounds(440, 306, 150, 30);
		add(comboPeriod);

		comboDow = new JComboBox();
		comboDow.setModel(new DefaultComboBoxModel(new String[] {"[\uADFC\uBB34\uC694\uC77C]", "\uD3C9\uC77C", "\uC8FC\uB9D0", "\uAE08,\uD1A0,\uC77C", "\uC6D4,\uC218,\uAE08", "\uD654,\uBAA9"}));
		comboDow.setBounds(440, 348, 150, 30);
		add(comboDow);

		comboTime = new JComboBox();
		comboTime.setModel(new DefaultComboBoxModel(new String[] {"[\uADFC\uBB34\uC2DC\uAC04]", "1\uC2DC\uAC04 ~ 3\uC2DC\uAC04", "3\uC2DC\uAC04 ~ 6\uC2DC\uAC04", "6\uC2DC\uAC04 ~ 8\uC2DC\uAC04", "8\uC2DC\uAC04 \uC774\uC0C1"}));
		comboTime.setBounds(440, 390, 150, 30);
		add(comboTime);

		comboRegion = new JComboBox();
		comboRegion.setModel(new DefaultComboBoxModel(new String[] {"[\uADFC\uBB34\uC9C0\uC5ED]", "\uAC15\uC6D0", "\uACBD\uAE30", "\uACBD\uB0A8", "\uACBD\uBD81", "\uAD11\uC8FC", "\uB300\uAD6C", "\uB300\uC804", "\uC11C\uC6B8", "\uC138\uC885", "\uC6B8\uC0B0", "\uC778\uCC9C", "\uC804\uB0A8", "\uC804\uBD81", "\uC81C\uC8FC", "\uBD80\uC0B0", "\uCDA9\uB0A8", "\uCDA9\uBD81"}));
		...
		modifyBackground.setIcon(new ImageIcon(ResumeModifyView.class.getResource("/resource/resumeE&M_Background.png")));
		modifyBackground.setHorizontalAlignment(SwingConstants.CENTER);
		modifyBackground.setBounds(0, 0, 1000, 600);
		add(modifyBackground);
	...
	public void settingData() {
		Resume temp = null;
		if ((temp = checkingController.sendResume()) != null) {
			...
			comboAcademic.setSelectedItem(temp.getAcademic());
			tfCareer.setText(temp.getCareer());
			comboTypeOccup.setSelectedItem(temp.getTypeOccup());
			comboPeriod.setSelectedItem(temp.getPeriodType());
			comboDow.setSelectedItem(temp.getDow());
			comboTime.setSelectedItem(temp.getTime());
			comboRegion.setSelectedItem(temp.getRegion());
		}
		else
		{
			System.out.println("이력서 불러오기 오류");
		}
	}
		...
		complete.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(!phoneNum.equals("") && typeOccup!="[근무형태]" && dow!="[근무요일]" && time!="[근무시간]") {
					resumeModifyController.modifyResume(name,age,gender,phoneNum,academic,career,typeOccup,periodType,dow,time,region);
